...
var js = @"
        ...
        document.getElementById('createCourseForm').addEventListener('submit', async function(e) {
            ...
            const payload = {
                IdAffectation: parseInt(document.getElementById('courseMatiereId').value),
                TypeCours: formData.get('TypeCours') || 'CM',
                Mode: 'Presentiel',
                Statut: 'Cree'
            };
        ...
        async function loadNiveaux() {
            try {
                ...
                niveaux.forEach(n => {
                    sel.innerHTML += `<option value=""${n.IdNiveau}"">${n.CodeNiveau}</option>`;
                });
            } catch (e) {
                console.error(e);
            }
        ...
        async function loadFilieres() {
            ...
            try {
                ...
                filieres.forEach(f => {
                    sel.innerHTML += `<option value=""${f.IdFiliere}"">${f.NomFiliere} (${f.CodeFiliere})</option>`;
                });
                sel.disabled = false;
            } catch(e) {}
        }
        ...
        async function loadMatieresNonPlanifiees() {
            ...
            try {
                ...
                data.forEach(m => {
                    sel.innerHTML += `<option value=""${m.IdAffectation}"">${m.NomMatiere} (${m.CodeMatiere}) - Prof: ${m.ProfesseurNom}</option>`;
                });
                sel.disabled = false;
            } catch(e) {
        ...
        function updateCreationSummary() {
            ...
            const m = currentMatieresOptions.find(x => x.IdAffectation === affId);
            const type = typeSelect.value || 'CM';
            
            if (m) {
                let vol = 0;
                if (type === 'CM') vol = m.HeuresCm;
                else if (type === 'TD') vol = m.HeuresTd;
                else if (type === 'TP') vol = m.HeuresTp;
                
                document.getElementById('creationSummary').innerHTML = `
                    Matière : <strong class=""text-indigo-700"">${m.NomMatiere} (${type})</strong> <br/>
                    Volume récupéré : <strong class=""text-indigo-700"">${vol}h</strong> <br/>
                    Enseignant assigné par admin : <strong class=""text-indigo-700"">${m.ProfesseurNom}</strong>
                `;
            }
        }
...
var file = ""Views/Home/Cours.cshtml"";
var text = File.ReadAllText(file);
// We replace the last </script>} with the new script logic
