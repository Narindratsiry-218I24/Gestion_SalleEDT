    ...
    public class EDTController : ControllerBase
    {
            ...
            string? cycle = null, string? niveau = null)
        {
            var query = db.Seances
                ...
                .Include(s => s.Cours).ThenInclude(c => c.Classe).ThenInclude(cl => cl.Semestre).ThenInclude(sem => sem.RefSemestre).ThenInclude(rs => rs.Niveau)
                .Include(s => s.Salle)
                .AsQueryable();
            ...
            if (!string.IsNullOrEmpty(niveau) && niveau != "all")
                query = query.Where(s => s.Cours.Classe.Semestre.RefSemestre.Niveau.CodeNiveau.Contains(niveau));

            var seancesList = query.ToList();

            var result = seancesList.Select(s => new {
                s.Id,
                s.Date,
                s.StartTime,
                s.EndTime,
                Course = new {
                    ...
                    Classe = s.Cours.Classe != null ? new { 
                        ...
                        Niveau = s.Cours.Classe.Semestre?.RefSemestre?.Niveau?.CodeNiveau
                    } : null
                },
